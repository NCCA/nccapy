...
def test_check_verts():
    ...
    assert [r == v for r, v in zip(result, obj.verts)]
...
def test_check_face_vert_only():
    ...
    assert obj.faces[0].vert[0] == 0
    assert obj.faces[0].vert[1] == 1
    assert obj.faces[0].vert[2] == 2
...
def test_check_face_vert_only_negative_index():
    ...
    for i in range(0, len(obj.faces)):
        assert obj.faces[i].vert[0] == idx
        assert obj.faces[i].vert[1] == idx + 1
        assert obj.faces[i].vert[2] == idx + 2
        assert obj.faces[i].vert[3] == idx + 3
        idx += 4
...
def test_build_obj():
    ...
    assert new.verts[0] == Vec3(2.0, 0.0, 0.0)
    assert new.verts[1] == Vec3(0.0, 4.0, 0.0)
    assert new.verts[2] == Vec3(-2.0, 0.0, 0.0)
